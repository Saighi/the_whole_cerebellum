...
def whole_model_sim(cere_info, input_dict):
    ...
    # correspond to the postsynaptic increase in calcium leading to plasticity
    new_event = dict()
    ...
    new_event["function"] = default_excitatory_behavior
    cere_events.append(new_event)
    # correspond to the actual plasticity mechanism depending on granule cells
    ...
    for event in cere_events:
        ...
        output_path = event[
            "output"]  # as there is only one output for each function we don't have to loop over output.
    ...
    while True:
        ...
        for neuron_pop in cere_state["neuron_pop"]:
            for local_variable in cere_state["neuron_pop"][neuron_pop]["local_variables"]:
                cere_state["neuron_pop"][neuron_pop]["local_variables"][local_variable][:] = \
                input_dict[neuron_pop][local_variable][t]

        for event in event_pointers:
            np.copyto(event["output"], event["function"](*event["args_tuple"])(event["output"]))

        for neuron_pop in cere_state["neuron_pop"]:
            cere_state["neuron_pop"][neuron_pop]["local_variables"]["activity"][:] = np.clip(
                cere_state["neuron_pop"][neuron_pop]["local_variables"]["activity"], a_min=0, a_max=None)
